...
# Really basic config handling, just enough to get the passwords out of
# the source code.
...
class mucBot(JabberBot):
    ...
    def __init__(self, username, password, res=None, debug=False, acceptownmsgs=False):
        super(mucBot, self).__init__(
            username, password, res, debug, acceptownmsgs)
        print self.res
...
#       self.connect().send(mess)
    ...
    def callback_message(self, conn, mess):
        ...
        if messtype == 'groupchat':
            ...
            # JID resource with our nick, as sender's real JID cannot be
...
def core():
    core_bot = mucBot(
        config.xmppUser, config.xmppPass, res=config.xmppResource, debug=True)
